...
private:
	enum class Deck { Normal, Review, Mastered };
...
QVariantMap Quiz::card() const
{
	...
	data["kanji"] = m_curCard->kanji;
	data["reading"] = m_curCard->reading;
	data["eigo"] = m_curCard->eigo;
	data["deck"] = [&]() -> QChar
			{
				switch (m_curCard->deck) {
				case Deck::Review: return 'R';
				case Deck::Mastered: return 'M';
				default: return 'N';
				}
			}();
...
void Quiz::writeDeck() const
{
	...
	for (auto& card : m_cards) {
		if (card.deck != Deck::Normal) {
			stream << card.kanji << QChar(':') << (card.deck == Deck::Review ? QChar('R') : QChar('M')) << QChar('\n');
		}
	}
}
...
{
	return QStringLiteral("%1 [/%2 ?=%3])").arg(m_viewedCards).arg(countVisibleCards()).arg(countReviewCards());
}

int Quiz::countVisibleCards() const
{
	return std::count_if(std::begin(m_cards), std::end(m_cards), std::bind(&Quiz::canShowCard, this, std::placeholders::_1));
}

int Quiz::countReviewCards() const
{
	return std::count_if(std::begin(m_cards), std::end(m_cards), [](const Card& c) { return c.deck == Deck::Review; });
}
